...
class AjaxSubmitDemo2 extends FormBase {
  ...
  public function setMessage(array $form, FormStateInterface $form_state) {
    ...
    $summ = $form_state->getValue('number_1') + $form_state->getValue('number_2');
    $response->addCommand(
      new HtmlCommand(
        '.result_message',
        '<div class="my_top_message">' . t('The results is @result', ['@result' => $summ]) . '</div>'),
    );

    $dispatcher = \Drupal::service('event_dispatcher');
    $event = new AjaxFormSubmitEvent($summ);
    $dispatcher->dispatch(AjaxFormSubmitEvent::EVENT_STARTED, $event);
